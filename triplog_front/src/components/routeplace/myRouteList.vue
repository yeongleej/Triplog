...
const myRecentRouteList = toRef(props, "myRouteList");
...
<template>
  <div class="route-container">
    ...
    <div v-if="myRecentRouteList.length == 0" class="route-content">
      <div class="route-detail">아직 루트가 없어요. 나만의 루트를 만들어 보세요!</div>
    </div>
    <div v-else class="route-content">
      <div class="route-item" v-for="(item, index) in myRecentRouteList" :key="index">
    ...
    <div class="myRoute-item">
      <router-link :to="{ name: 'my-route' }" class="myRoutesbtn">내루트 더보기..</router-link>
    </div>
  </div>
</template>
